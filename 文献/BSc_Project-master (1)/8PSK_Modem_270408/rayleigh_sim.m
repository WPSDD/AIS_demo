...
% simulates rayleigh fading for the number of paths.
% simulate with 4 paths

function rt = rayleigh_sim(signal,sps)

% define global variables - so that it can be accessed from workspace
%global ray1 ray2 ray3 ray1_g ray2_g ray3_g
%global sig1 sig2 sig3

   % delay tx signals over 4 paths.
    delay1 = 0; delay2 = 0.5 ; delay3 = 0.5;
...
% -----------------------------------------------------    
    ...
    %ray1 = abs(randn(1,length(signal))+ j*randn(1,length(signal)));  % ray1 = kron(ray1,ones(1,sps));
   ...
   %gain2 = sum(ray2)/sum([ray1 ray2 ray3];
   %gain3 = sum(ray3)/sum([ray1 ray2 ray3];
