...
gmsk_dpl_tdly = [2*rand(1, round(delay/dt))-1, gmsk_dpl];   %信号时延
gmsk_dpl_tdly = gmsk_dpl_tdly(mod(length(gmsk_dpl_tdly), 25)+1: end);
gmsk_rcv = awgn(gmsk_dpl_tdly, 15, 'measured');  %信号经过AWGN信道后信噪比为15dB
...
%产生两路正交因子进行下变频
cn = cos(2*pi*(fc1+f_dpl_est)*(0:dt:dt*(length(gmsk_rcv)-1)));           %同相分量的下变频正交因子
sn = sin(2*pi*(fc1+f_dpl_est)*(0:dt:dt*(length(gmsk_rcv)-1)));           %正交分量的下变频正交因子
...
I_de = I_de0;                                                               %抽取滤波后的同相信号分量
Q_de = Q_de0;                                                               %抽取滤波后的正交信号分量
